...
export function VerticalTreeChart({
  ...
  const LinkComponent = getLinkComponent({ layout, linkType, orientation });
	...
	console.log("issuesByEpic:",issuesByEpic)
	console.log("rootEpicIssue:",rootEpicIssue)
	...
	const transformDataToTree = ({ epic, issues }) => {
		...
		return {
			...
			children: issues.map(issue => ({ 
				...
				children: issue.fields.subtasks.map(subtask => {
					return { name: subtask.key, children: [], issuelinks: [] };
				}) 
			}))
		}
	}
  ...
  if ((transformedTreeData.length === 0) || rootEpicIssue.errorMessages?.length > 0) {
	return null;
  }
            ...
            {(tree) => (
              ...
              <Group top={origin.y} left={origin.x}>
                ...
                {tree.descendants().map((node, index) => {
                  ...
                  return (
	<Tooltip content="wow" interactive key={index}>
					
		<a key={index} data-testid="issue-field-summary.ui.inline-read.link-item" data-is-router-link="true" data-vc="link-item"  aria-disabled="false" href="https://gilitz.atlassian.net/browse/ET-2" target='_parent'>
